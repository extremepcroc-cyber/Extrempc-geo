# FGG MAD68 HE Flagship V2 RGB Hot-Swappable Wired Mechanical Gaming Keyboard - Black, Magneto Gaming Switch, 68 keys

**Price:** NZD $99.00 (incl. GST) (on sale from $129.00)
**SKU:** KEYFGGM68FBM
**MPN:** MAD 68 HE flagship V2
**URL:** https://www.extremepc.co.nz/fgg-mad68-he-flagship-v2-wired-mechanical-gaming-keyboard-black-magneto-gaming-switch-68-keys/
**Status:** In stock — Only a few left (BC API verified 2026-08-26: OH=1)

## Quick Specs
- **Connection:** Wired (USB-C)
- **Switch Type:** Magneto Gaming Switch (magnetic Hall-effect — HE)
- **Hot-Swappable:** Yes
- **Backlight:** RGB
- **Keys:** 68 (65% layout)
- **Form Factor:** 65%
- **Ergonomic:** No
- **Display:** No
- **Color:** Black

## Notes
- Magnetic Hall-effect (HE) switch platform — supports rapid trigger / adjustable actuation point, well suited for fighting games (SF6, Tekken, GGST)
- Hot-swappable — swap to other magnetic or conventional switches
- Flagship V2 revision of the MAD68 HE line (distinct from FGG MAD68 Pro [KEYFGGM68PBJ] with Gateron Jade Esport Pro)
- New arrival — added to stock 2026-08-26

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- 2-year NZ warranty support
