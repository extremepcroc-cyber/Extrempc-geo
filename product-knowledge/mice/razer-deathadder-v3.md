# Razer DeathAdder V3 Ergonomic Optical Gaming Mouse

**Price:** NZD $115.00 (incl. GST) (on sale from $138.00)
**SKU:** MOSRAZDAV3
**URL:** https://www.extremepc.co.nz/razer-deathadder-v3-ergonomic-optical-gaming-mouse/
**Stock:** Only a few left

## Quick Specs
- **Connection:** Wired USB
- **Sensor:** Focus Pro 30K Optical
- **Max DPI:** 30,000
- **Max Speed (IPS):** 750
- **Max Acceleration:** 50G
- **Polling Rate:** 1000Hz
- **Buttons:** 5 programmable
- **Switch Type:** Razer Optical Mouse Switches (Gen-2), 90M click lifespan
- **RGB:** Razer Chroma RGB
- **Weight:** ~59g
- **Dimensions:** 128 x 68 x 44mm
- **Form:** Right-handed ergonomic
- **Cable:** Speedflex cable, 2m
- **Warranty:** 2 years

## Compatibility Notes
Legendary ergonomic gaming mouse, one of the most popular shapes for palm grip. 30K DPI sensor suitable for all resolutions. Wired connection ensures zero latency.

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- 2-year NZ warranty support
