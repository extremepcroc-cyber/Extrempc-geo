# Colorful GeForce RTX 3050 6GB V4-V Graphics Card

**Price:** NZD $419.75 (incl. GST) (on sale from $439.00)
**SKU:** GPUCOL356V4
**URL:** https://www.extremepc.co.nz/rtx-30-series/colorful-geforce-rtx-3050-6gb-v4-v-graphics-card/
**Stock:** Plenty in stock

## Quick Specs
- GPU: NVIDIA GeForce RTX 3050 (Ampere)
- Memory: 6GB GDDR6
- Memory Bus: 96-bit
- TDP: ~130W
- Recommended PSU: 450W 80+ Bronze
- Power Connectors: 1x 6-pin PCIe
- Target Resolution: 1080p medium/low settings
- AIB Variant: V4-V (2-slot, compact)

## Compatibility Notes
- Fits most ATX and M-ATX cases (check case GPU clearance)
- Works with PCIe 3.0, 4.0, and 5.0 motherboards
- Entry-level DLSS support, CUDA ecosystem
- Budget option for 1080p gaming on older or lower-wattage systems

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
