# PNY GeForce RTX 5080 Slim OC Dual Fan 16GB GDDR7 Graphics Card

**Price:** NZD $2899.00 (incl. GST)
**SKU:** GPUPNY58SDOC
**MPN:** VCG508016DFSXPB1-O
**URL:** https://www.extremepc.co.nz/csv-import/pny-nvidia-geforce-rtx-5080-slim-oc-dual-fan-16gb-gddr7-graphics-card-pcie-5-0-2-slot-1x-16-pin-power-minimum-850w-psu/
**Stock:** — (see status)
**Status:** OUT OF STOCK (BC API verified 2026-09-02, all warehouses 0)

## Quick Specs
- GPU: NVIDIA GeForce RTX 5080 (Blackwell)
- Memory: 16GB GDDR7
- Memory Bus: 256-bit
- TDP: 360W
- Recommended PSU: 850W 80+ Gold
- Power Connectors: 1x 12VHPWR (16-pin)
- Target Resolution: 4K ultra
- AIB Variant: Slim OC (PNY's compact dual-fan design with factory OC)

## Compatibility Notes
- Slim dual-fan design — excellent for compact builds, check case GPU clearance
- Works with PCIe 4.0 and PCIe 5.0 motherboards
- CUDA ecosystem for AI/creative workloads, DLSS 4 Multi Frame Generation
- 16GB VRAM suitable for 4K gaming and AI workloads
- PNY's Slim OC line is known for compact form factor without sacrificing performance

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
