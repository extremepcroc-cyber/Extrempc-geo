# MSI GeForce RTX 5070 Ti VENTUS 3X OC PLUS 16GB Graphics Card

**Price:** NZD $2,519.01 (incl. GST)
**SKU:** GPUMSI5070TV316O
**URL:** https://www.extremepc.co.nz/msi-geforce-rtx-5070-ti-ventus-3x-oc-plus-16gb-graphics-card/
**Stock:** Only a few left in stock

## Quick Specs
- GPU: NVIDIA GeForce RTX 5070 Ti (Blackwell)
- Memory: 16GB GDDR7
- Memory Bus: 256-bit
- TDP: ~300W
- Recommended PSU: 750W 80+ Gold
- Power Connectors: 2x 8-pin or 1x 12VHPWR
- Target Resolution: 1440p ultra / 4K high
- AIB Variant: VENTUS 3X OC PLUS (3-slot, factory overclocked)

## Compatibility Notes
- 3-slot design — ensure case GPU clearance ~300mm+
- Works with PCIe 4.0 and PCIe 5.0 motherboards
- DLSS 4 Multi Frame Generation, Ray Tracing Gen 5
- 16GB VRAM — handles 4K textures and productivity workloads

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
