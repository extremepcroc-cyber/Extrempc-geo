# Gigabyte NVIDIA GeForce RTX 5070 Ti WINDFORCE OC V2 16GB GDDR7 Graphics Card

**Price:** NZD $2,519.01 (incl. GST)
**SKU:** GPUGIG57TW2O
**URL:** https://www.extremepc.co.nz/csv-import/gigabyte-nvidia-geforce-rtx-5070-ti-windforce-oc-v2-16gb-gddr7-graphics-card-pcie-5-0-2-5-slot-1x-16-pin-power-minimum-750w-psu/
**Stock:** Only a few left in stock

## Quick Specs
- GPU: NVIDIA GeForce RTX 5070 Ti (Blackwell)
- Memory: 16GB GDDR7
- Memory Bus: 256-bit
- TDP: 300W
- Recommended PSU: 750W 80+ Gold
- Power Connectors: 1x 12VHPWR (16-pin)
- Target Resolution: 1440p ultra / 4K high settings
- AIB Variant: WINDFORCE OC V2 (Gigabyte's updated mainstream lineup, 2.5-slot design)

## Compatibility Notes
- 2.5-slot design — check case GPU clearance
- Works with PCIe 4.0 and PCIe 5.0 motherboards
- CUDA ecosystem for AI/creative workloads, DLSS 4 Multi Frame Generation
- V2 revision with improved cooling over original WINDFORCE

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
