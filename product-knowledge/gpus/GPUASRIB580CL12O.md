# ASRock Intel Arc B580 Challenger OC 12GB GDDR6 PCIe 4.0 Graphics Card

**Price:** NZD $598.00 (incl. GST) (on sale from $619.00)
**SKU:** GPUASRIB580CL12O
**URL:** https://www.extremepc.co.nz/shop-all/asrock-intel-arc-b580-challenger-oc-12gb-gddr6-pcie-4-0-graphics-card/
**Stock:** We have plenty in stock

## Quick Specs
- GPU: Intel Arc B580 (Battlemage)
- Memory: 12GB GDDR6
- Memory Bus: 192-bit
- TDP: 190W
- Recommended PSU: 550W 80+ Gold
- Power Connectors: 1x 8-pin
- Target Resolution: 1080p ultra / 1440p high
- AIB Variant: Challenger OC (ASRock's mainstream dual-fan design with factory OC)

## Compatibility Notes
- Dual-fan design — check case GPU clearance (typically ~240-260mm)
- Works with PCIe 4.0 motherboards
- XeSS for upscaling, Intel's AI-driven super sampling
- Best value for 1080p gaming on a budget
- Intel Arc drivers have improved significantly — check for latest updates

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
