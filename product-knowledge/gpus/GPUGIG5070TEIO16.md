# Gigabyte GeForce RTX 5070 Ti EAGLE OC ICE SFF 16GB Graphics Card

**Price:** NZD $2360.00 (incl. GST, list — sale $2254.00 seen 2026-09-07 BC calc)
**SKU:** GPUGIG5070TEIO16
**URL:** https://www.extremepc.co.nz/gigabyte-geforce-rtx-5070-ti-eagle-oc-ice-16gb-gddr7-graphics-card/
**Stock:** OUT OF STOCK (verified 2026-09-07, BC API OH=0, all warehouses 0)

## Quick Specs
- GPU: NVIDIA GeForce RTX 5070 Ti (Blackwell)
- Memory: 16GB GDDR7
- Memory Bus: 256-bit
- TDP: 300W
- Recommended PSU: 750W 80+ Gold
- Power Connectors: 1x 12VHPWR (16-pin)
- Target Resolution: 1440p ultra / 4K high settings
- AIB Variant: EAGLE OC ICE (white-themed compact design, SFF form factor)

## Compatibility Notes
- SFF (Small Form Factor) design — check case GPU clearance (likely ~260mm length)
- Works with PCIe 4.0 and PCIe 5.0 motherboards
- CUDA ecosystem for AI/creative workloads, DLSS 4 Multi Frame Generation
- White color — matches white-themed builds, compact for ITX/M-ATX cases

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- Expert build service available ($80 build fee)
- 3-year NZ manufacturer warranty
