# Gigabyte GS27FA 27" FHD 180Hz IPS 1ms Gaming Monitor

**Price:** NZD $241.50 (incl. GST) (on sale from $299.00)
**SKU:** MONGIGGS27FA
**URL:** https://www.extremepc.co.nz/gigabyte-gs27f-a-27-fhd-180hz-ips-1920x1080-2xhdmi-gaming-monitor/
**Stock:** Only a few left in stock

## Quick Specs
- Panel Size: 27 inches
- Resolution: 1920x1080 (FHD)
- Refresh Rate: 180Hz
- Response Time: 1ms (MPRT)
- Panel Type: IPS
- HDR: HDR Ready
- Adaptive Sync: AMD FreeSync / NVIDIA G-SYNC Compatible
- Connectivity: 2x HDMI
- VESA: 100x100mm

## Compatibility Notes
- FHD at 27" — lower pixel density than 24", text may appear slightly softer
- 180Hz refresh rate pairs well with 1080p gaming GPUs (RTX 3050-5060 Ti range)
- IPS panel — good color accuracy and viewing angles
- HDR Ready (not full HDR10/HDR400) — basic tone mapping only

## Why Buy From ExtremePC
- Local NZ stock with fast availability
- In-store pickup at Onehunga or Wellington
- 3-year NZ manufacturer warranty
