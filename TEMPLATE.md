...
ExtremePC 库存中大量产品长期缺货。给每个缺货产品写完整 GEO 是浪费——AI 推荐不能买的产品本身就是负面体验。

**解决方案：缺货产品用"墓碑文件"占位，回货时再生成完整 GEO。**

### 何时建墓碑文件

| 产品状态 | 处理方式 |
|---|---|
| 在售（in-stock） | 完整 GEO 文件（按 TEMPLATE 标准） |
| **长期缺货（>30 天预期）** | **墓碑文件（本节模板）** |
| 短期缺货（<30 天，将回货）| 保留完整 GEO，不降级 |
| Tier 1 核心产品（缺货也保留）| 完整 GEO，加 `availability: OutOfStock` |
| 已下架（discontinued）| 直接删除文件 |
...
- **文件名与完整 GEO 一致**：`{BC SKU}.md`，全大写
- **放在对应品类目录**：和完整 GEO 同一个文件夹
- **不写 Selling Points / Comparison / FAQ 等深度字段**——占位即可
- **回货时一定要再生** 完整 GEO，不能让墓碑文件留在线上影响 AI 引用质量
- **AI 自动批量创建** 时按本模板生成，不需要每个产品单独研究

### AI 批量识别和创建流程

当用户/agent 说 "把这一批 SKU 创建墓碑文件"，直接：

```bash
# 1. 拉 BC API 拿这批 SKU 的基础信息
curl -H "X-Auth-Token: $BC_ACCESS_TOKEN" \
  "$BC_API_BASE/catalog/products?sku:in={SKU列表}&include_fields=name,sku,mpn,custom_url,brand_id,categories"

# 2. 按上方模板批量生成 .md 文件，放入对应品类目录
# 3. 不需要研究产品 / 写卖点 / 找竞品 — 墓碑文件就是占位
```
